  ...
  start (userConfig) {
    ...
    this._startPromise = makeDir(this.params.workDir)
      .then(() => {
        ...
        return this.page.route('**', this.requestHandler);
      })
      .catch((e) => {
        logger(`Unknown error: ${e.message}`)
      });
  ...
  stop () {
    ...
    return this._startPromise
      .then(() => {
        ...
        return this.page.unroute('**', this.requestHandler);
      })
      .then(() => {
        logger('requestInterception disabled, fail timeout cleared');

        this.params.pagesSet.delete(this.page);
